...
const plugins = pages.map(page => new HtmlWebpackPlugin({
    template: resolve(__root__, `src/pages/${page.name}/${page.template ?? 'template'}.html`),
...
// 入口配置
const entry = {
    'common-style': resolve(__root__, `src/assets/style/public.less`)
}
...
for(const page of pages) {
    ...
    const pagePath = resolve(__root__, 'src/pages', page.name)
